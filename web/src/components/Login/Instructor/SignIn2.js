...
class SignIn extends Component{
  ...
  onSubmit(e){
    e.preventDefault();
		axios.post("http://103.210.75.167:3004/api/instructor-authenticate",this.state).
            then(res=>{
                if(res.data.logged)
                {
                    ...
                    this.setState({error:false, islogged:true,user:res.data.instructor});
  ...
  render(){
    return(
      <div>
         {this.state.islogged?<Redirect to={{pathname: "/lms/instructor/dashboard", state:this.state.user}} ></Redirect>:null}
      <div>
      ...
      <div className="col-sm-9 col-md-7 col-lg-5 mx-auto">
        <div className="card card-signin my-5">
          <div className="card-body">
            <h2 className="card-title text-center"><b>Sign In As Instructor</b></h2>
            <center>
            ...
            <form className="form-signin" onSubmit={this.onSubmit} ><input type="hidden" name="_csrf" defaultValue="8988e4bb-70cc-457f-86b6-d73bf13d30ac" />
              <div className="form-label-group">
                <input type="email" id="inputEmail" className="form-control" placeholder="Email address" required autoFocus name="email" onChange={this.onEmailChange}  value={this.state.email} />
        ...
        {this.state.error==true?<p style={{margin:5}}>Invalid Email or Password</p>:null}
              <hr className="my-4" />  
              <Link to="/lms/user/register" style={{fontSize: '20px'}}>Sign Up</Link><Link className="float-right" to="/lms/user/forgot-password" style={{fontSize: '20px'}}>Forgot password?</Link>
            </form>
          </div>
        </div>
      </div>
